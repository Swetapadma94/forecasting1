...
cocacola<-read_excel("E:\\Assignment\\forecasting\\coca.xlsx")
View(cocacola)
# Converting data into time series object
coc<-ts(cocacola$Sales,frequency = 4,start=c(86))
View(coc)
plot(coc)

# dividing entire data into training and testing data 
train=coc[1:38]
test=coc[39:42]# Considering only 4 Quarters of data for testing because data itself is Quarterly
# seasonal data

# converting time series object
trainn<-ts(train,frequency = 4)
testt<-ts(test,frequency = 4)
...
hwab_mape<-MAPE(hwab_pred$fit,testt)*100
# with alpha = 0.2, beta = 0.1, gamma = 0.1 
# Assuming time series data has level,trend and seasonality 
hw_abg<-HoltWinters(trainn,alpha = 0.2,beta = 0.1,gamma = 0.1)
...
hwabg_mape<-MAPE(hwabg_pred$fit,testt)*100
# Without optimum values 
hw_na<-HoltWinters(trainn,beta = F,gamma = F)
hwna_pred<-data.frame(predict(hw_na,n.ahead = 4))
hwna_pred
plot(forecast(hw_na,h=4))
hwna_mape<-MAPE(hwna_pred$fit,testt)*100
hw_nab<-HoltWinters(trainn,gamma=F)


hwnab_pred<-data.frame(predict(hw_nab,n.ahead=4))
hwnab_pred
plot(forecast(hw_nab,h=4))
hwnab_mape<-MAPE(hwnab_pred$fit,testt)*100


hw_nabg<-HoltWinters(trainn)

hwnabg_pred<-data.frame(predict(hw_nabg,n.ahead =4))
hwnabg_pred
plot(forecast(hw_nabg,h=4))
hwnabg_mape<-MAPE(hwnabg_pred$fit,testt)*100
